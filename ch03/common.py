# ============================================================================
# common.py —— ch03 公共配置模块
# 职责：统一提供 DeepSeek 模型实例 + 文本预览工具 + .env 自动加载。
# 所有主脚本（01~04）与测试都 import 本模块，避免重复代码。
# ============================================================================

# 模块级文档字符串：说明本文件用途与运行方式
"""ch03 公共配置：DeepSeek 官方接口 + deepseek-v4-flash。

ch03 有独立 .venv（依赖与 ch02 一致），并在 import 时自动加载 ch03/.env，
因此直接在 ch03/ 目录下 `uv run <脚本>.py` 即可，无需 --project / --env-file。
"""
...
# 作用：把 ch03/.env 里的 KEY=VALUE 注入进程环境变量，使脚本无需手动 export。
# ---------------------------------------------------------------------------
